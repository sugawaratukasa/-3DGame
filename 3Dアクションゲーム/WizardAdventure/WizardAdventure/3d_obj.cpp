...
void C3D_Obj::UnloadTexture(void)
{
	for (int nCnt = INIT_INT; nCnt < TYPE_MAX; nCnt++)
	{
		...
		{
			// NULLでない場合
			if (m_apTexture[nCnt][nCntMat] != NULL)
			{
				// テクスチャRelease
				m_apTexture[nCnt][nCntMat]->Release();

				// m_pTextureをNULLに
				m_apTexture[nCnt][nCntMat] = NULL;
...
HRESULT C3D_Obj::Init(void)
{
	...
	{
		// テクスチャのバインド
		m_pModel->BindTexture(m_apTexture[m_Type][nCntMat], nCntMat);
	}
